...
In practical terms, this means that spectral methods can achieve a level of precision with a few dozen degrees of freedom that a finite difference scheme might require thousands of grid points to match. While a fourth-order finite difference method implies that the error $epsilon tilde O(N^(-4))$, a spectral method boasts $epsilon tilde O(c^(-N))$ for some constant $c > 1$ @KreissOliger1972. When the solution is analytic on a neighbourhood of the domain, the convergence is geometric at this rate; for an entire solution it is faster still, and the error plunges into the "spectral valley" until it reaches the floor of machine precision.
...
Spectral representations have been used for analytic studies of differential equations since the days of Fourier in the early nineteenth century. The idea of employing them for _numerical_ computation, however, emerged much later. Lanczos, in the 1930s, pioneered the use of Chebyshev expansions for solving ordinary differential equations numerically @Lanczos1938. In the two decades that followed, a British school centred on the National Physical Laboratory (Clenshaw, and later Fox, Mason, and Elliott) turned Chebyshev series into a practical instrument of numerical computation, giving us the Clenshaw--Curtis quadrature rule and the stable recurrences for summing Chebyshev expansions @Fox1968a. This approach remained the province of specialists until the early 1970s, when Kreiss and Oliger introduced the pseudospectral method for partial differential equations @KreissOliger1972. Their innovation was to work with function values at grid points rather than expansion coefficients, dramatically simplifying the treatment of nonlinear terms. At very nearly the same time, Orszag applied these ideas to fluid dynamics, and it is largely through his work that spectral methods became widely known @Orszag1971. The scaling of these methods to large problems was then greatly advanced by the fast Fourier transform algorithm, rediscovered by Cooley and Tukey in 1965 @Cooley1965, which reduced the cost of transforming between physical and spectral space from $O(N^2)$ to $O(N log N)$ operations. This confluence of theoretical insight and algorithmic efficiency launched spectral methods into the mainstream of computational science.
