...
Fractional derivatives, such as those defined by Riemann--Liouville or Caputo, are non-local integro-differential operators; the derivative of a function at a point $x$ depends on the entire history of the function across the domain. Local numerical methods, such as finite differences, struggle immensely with fractional operators, as the required non-local stencils completely destroy matrix sparsity and computational efficiency.

Spectral methods, however, are inherently global and thus perfectly suited to fractional calculus. By expanding a function in a global basis of specialised orthogonal polynomials --- such as shifted Legendre, Jacobi, or Vieta--Lucas polynomials --- the complex non-local fractional derivative can be computed exactly and analytically in coefficient space. Recent advancements utilising fractional-order integral operational matrices allow complex FDEs, such as the Bagley--Torvik equation, to be transformed into highly accurate algebraic matrix equations, entirely bypassing the need for traditional, error-prone time-stepping schemes. Spectral methods are now unequivocally the preeminent numerical tool for high-fidelity fractional physics.
