...
A note on scope is in order before we proceed. This Afterword was first drafted when the manuscript ended around its fifteenth chapter; the four chapters that have been added since --- @ch-linear-eigen on linear spectral eigenproblems, @ch-coord-transforms on coordinate transformations and mapped spectral methods, @ch-unbounded on spectral methods on unbounded intervals, and the closing bonus @ch-special-tricks on special tricks for the spectral researcher --- now bring much of what the original Afterword treated as 'future reading' inside the body of the text itself. Where appropriate, this Afterword has been brought forward to acknowledge those additions. Several of the topics described below as 'beyond the scope of the present text' are now in the present text; the cross-references make this explicit. The remaining Afterword content --- pure Galerkin and the Sobolev framework, ultraspherical sparse spectral methods, spectral elements, shock-capturing, exact dealiasing, and the modern frontiers (fractional calculus, general relativity, operator learning, quantum spectral algorithms) --- still constitutes genuinely external material that the curious researcher can profitably explore from here.
