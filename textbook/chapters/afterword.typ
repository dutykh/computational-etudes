...
#dropcap[The emphasis throughout the preceding computational études has been resolutely placed on the practical resolution of differential equations. The overriding objective of the text has been to demonstrate that the elementary tools of spectral methods can be deployed with relative ease to obtain high-accuracy numerical results across a diverse array of scientific domains. By prioritising computational implementation, the text has favoured physical-space formulations, explicit matrix operations, and canonical domain geometries. This pedagogical style, while highly effective for establishing initial intuition and algorithmic fluency, inherently de-emphasises certain critical dimensions of the discipline. Most regrettably, such an approach provides an incomplete indication of the profound mathematical depth and algorithmic maturity that the field of spectral methods has achieved over the past half-century.]

The landscape of spectral approximation is vast, extending far beyond the foundational collocation techniques on Chebyshev and Fourier grids presented in the main text. By the time spectral methods entered their "modern" era --- a quarter-century after the foundational works of the 1970s --- a remarkable body of mathematics had been formalised to establish their rigorous foundations. The purpose of this concluding chapter is to systematically survey the expansive territories that lie beyond the present text, bridging the gap between practical computational exercises and the state-of-the-art frontiers of applied mathematics. This exposition will detail the rigorous functional analysis underpinning spectral convergence, the advanced algorithmic refinements necessary for large-scale stability, the treatment of discontinuous phenomena in fluid mechanics, and the modern intersection of spectral theory with fractional calculus and operator learning.

A note on scope is in order before we proceed. Several of the topics surveyed below are treated in full within the body of the text: linear spectral eigenproblems (@ch-linear-eigen), coordinate transformations and mapped spectral methods (@ch-coord-transforms), spectral methods on unbounded intervals (@ch-unbounded), and the special tricks of the closing bonus chapter (@ch-special-tricks); the cross-references make this explicit. The remainder, namely pure Galerkin and the Sobolev framework, ultraspherical sparse spectral methods, spectral elements, shock-capturing, exact dealiasing, and the modern frontiers of fractional calculus, general relativity, operator learning, and quantum spectral algorithms, constitutes genuinely external material that the curious researcher can profitably explore from here.
...
This material is developed at length in two of the longest chapters of the book. @ch-coord-transforms (Coordinate Transformations and Mapped Spectral Methods) develops the systematic theory of one-dimensional algebraic and trigonometric maps, with explicit études on the arctan/tan periodic map, the algebraic semi-infinite map, and the Kosloff--Tal-Ezer redistribution; an adaptive-mapping capstone (§19.8) closes the chapter with the arclength and parametric strategies of @Boyd2000, presented as a project rather than a routine étude. @ch-unbounded (Spectral Methods on Unbounded Intervals) then takes the reader through the Hermite, Laguerre, sinc, rational Chebyshev $T B_n$ and $T L_n$, the Yoshida jet, and the Weideman--Cloot sinh-mapping families, with computational evidence at every step. The treatment of @ch-unbounded follows Boyd's framework @Boyd2000 closely; the lessons that 'one parameter $N$ is never enough on an unbounded interval' and 'the basis must mirror the asymptotic personality of the solution' are made operational there in a way that the bare summary of this Afterword cannot do.
...
Standard numerical shooting methods often fail to resolve these complex frequencies due to numerical dissipation. Modern research utilises Chebyshev collocation-type spectral methods combined with polynomial eigenvalue solvers to compute the QNMs of non-commutative geometry-inspired black holes and massive static phantom wormholes. The exponential convergence of the spectral method allows physicists to accurately resolve the boundary conditions at the event horizon and spatial infinity simultaneously, validating quantum-inspired modifications to classical Schwarzschild metrics. The reader interested in the algorithmic substrate of these calculations should consult @ch-linear-eigen of the present text, which treats linear spectral eigenproblems systematically --- including the QR/QZ machinery, inverse iteration with shift, parameter sweeps and tracking of nearly-degenerate branches, the diagnosis of physically spurious modes, and the complex-plane detour technique used to handle interior coefficient singularities. The complex-plane detour étude (@etude-complex-detour) is in particular the same technique on which much QNM work in non-commutative black-hole geometries rests.
...
A final word on the bonus @ch-special-tricks. That chapter, drawn directly from the special-tricks chapters of @Boyd2000, is deliberately positioned outside the main lecture sequence: it is the chapter that addresses the moment, two or five years from now, when a research problem refuses to yield to the standard machinery and the reader must improvise. Sideband truncation around a high carrier mode, singularity subtraction at corners, radiation-augmented bases for non-decaying scattering tails, Lanczos economization and Ioakimidis' non-iterative Chebyshev root, the Fourier-multiplier Hilbert transform, the six precepts of small-$N$ symbolic Galerkin work, and the Lanczos $tau$ philosophy of solving exact equations on nearby modified problems --- each of these is an instance of the same governing principle: a spectral method becomes elegant when the discretisation mirrors the structure of the solution. We hope the reader will return to that chapter when the moment of need arrives, and find there both the specific recipes and the diagnostic habit of mind that the moment requires.
