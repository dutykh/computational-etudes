...
#chapter-abstract(keywords: [Boundary conditions · Lifting technique · Tau method · Robin conditions · Quadratic eigenvalue problems · Quasinormal modes])[
Earlier chapters imposed only the simplest boundary condition --- homogeneous Dirichlet data enforced by deleting the first and last rows and columns of the differentiation matrix. Real applications demand much more: inhomogeneous Dirichlet data, Neumann and Robin conditions, time-dependent and nonlinear boundaries, and even conditions that depend on the unknown eigenvalue. This chapter organises these cases under two systematic strategies --- restricting the interpolant so that the conditions hold automatically (Method I), and replacing equations so that the conditions are enforced directly (Method II, the tau approach). The lifting technique reduces inhomogeneous Dirichlet problems to homogeneous ones, extending naturally to time-dependent boundary data through an added source term. Neumann, Robin, and mixed conditions follow by boundary-row replacement; nonlinear conditions are handled by coupling Newton iteration to the spectral discretisation; and Kronecker products carry every device into two dimensions. The chapter culminates in quadratic eigenvalue problems generated by frequency-dependent boundary conditions, with application to the quasinormal modes of black holes --- a vivid demonstration that careful boundary treatment turns the spectral method into a research-grade tool.
]
