...
The literature on spectral methods identifies two fundamentally different philosophies for imposing boundary conditions @Trefethen2000 @Boyd2000 @Canuto2006. Understanding the distinction is essential for choosing the right approach in any given problem.
...
For inhomogeneous Dirichlet conditions $u(a) = alpha$, $u(b) = beta$, the restriction is achieved through _lifting_: one defines a simple function $w(x)$ satisfying the boundary data and solves for the remainder $v = u - w$ with homogeneous conditions. The original solution is then reconstructed as $u = v + w$.
...
where $sigma > 0$ is a radiation parameter. The $u^4$ nonlinearity means that neither Method I (lifting) nor a simple tau row replacement suffices: the boundary condition must be solved iteratively at each time step.
...
where $times.o$ denotes the Kronecker product. The solution vector is formed by stacking the columns (or rows) of the two-dimensional grid values into a single vector of length $(N + 1)^2$.
...
The treatment of boundary conditions in spectral methods has evolved considerably since the early formulations of the 1970s, and the literature reflects a rich interplay between mathematical analysis, numerical algorithm design, and physical application.

The _tau method_ was introduced by Lanczos @Lanczos1938 in the context of polynomial approximation, where the highest-order expansion coefficients are modified to satisfy boundary constraints. The modern collocation variant --- replacing boundary rows of the discrete operator --- was developed systematically in the spectral methods literature by Gottlieb and Orszag @GottliebOrszag1977 and codified by Boyd @Boyd2000, who provides an encyclopaedic treatment of boundary condition imposition strategies including basis recombination, penalty methods, and boundary bordering. The comprehensive monographs by Canuto _et al._ @Canuto2006 and Shen, Tang, and Wang @ShenTangWang2011 give rigorous error analysis for various boundary condition formulations in both Galerkin and collocation settings.

Trefethen's _Spectral Methods in MATLAB_ @Trefethen2000, particularly Chapter 13, is the direct inspiration for the material in this chapter. His Programs 32--37 demonstrate lifting, tau row replacement, and mixed-dimension discretisations (Fourier in one direction, Chebyshev in the other) with remarkable economy of code.

The Allen--Cahn equation @AllenCahn1979 has become a standard testbed for numerical methods handling nonlinear reaction-diffusion dynamics. Spectral discretisations for Allen--Cahn and the related Cahn--Hilliard equation are analysed in detail by Shen and Yang, who develop energy-stable schemes that preserve the variational structure of the continuous problem. The metastability phenomenon --- exponentially long interface lifetimes --- is analysed rigorously by Carr and Pego, who show that the lifetime scales as $exp(C \/ epsilon)$ for small $epsilon$.

The _quadratic eigenvalue problem_ (QEP) that arises from frequency-dependent boundary conditions is a well-studied topic in numerical linear algebra. The comprehensive survey by Tisseur and Meerbergen @Tisseur2001 covers linearisation techniques, conditioning analysis, and algorithmic considerations. The companion linearisation used in Étude 13.6 is the simplest and most widely used approach; alternatives include the second companion form and the palindromic linearisation, which preserve structural properties of the original QEP.

_Quasinormal modes_ of black holes have been studied intensively since the work of Vishveshwara (1970) and Press (1971). The Pöschl--Teller potential @PoschlTeller1933 was recognised as an exactly solvable model for black hole QNMs by Ferrari and Mashhoon @FerrariMashhoon1984, who derived closed-form expressions for the complex frequencies. Modern reviews of QNM physics and computation include Nollert @Nollert1999, Kokkotas and Schmidt @KokkotasSchmidt1999, Berti _et al._ @Berti2009, and Konoplya and Zhidenko @Konoplya2011. Spectral methods have proven particularly effective for QNM computation: the Chebyshev collocation approach used in this chapter has been applied to a wide range of black hole and wormhole geometries by Batic and Dutykh @Batic2024 @Batic2024b @Batic2024c @Batic2025 @Batic2025a @Batic2025b, demonstrating spectral convergence of complex eigenfrequencies for Schwarzschild, noncommutative-geometry-inspired, and Lee--Wick black holes, as well as Morris--Thorne wormholes.
