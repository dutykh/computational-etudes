...
a well-behaved constant. But computing the individual terms requires evaluating $u_r \/ r = -2 r \/ r = -2$, which at $r = 0$ presents a $0\/0$ indeterminate form. Any numerical method that places a grid point at $r = 0$ must somehow evaluate these singular-looking terms, and doing so stably is a nontrivial challenge.
...
where $h = 2 pi \/ M$ is the angular spacing. This matrix was encountered in @ch-differentiation in the context of periodic differentiation and can equivalently be computed via FFT multiplication by $-k^2$ in Fourier space, as in @ch-fourier-grids.
...
As an additional check, we solve the same problem with a _centred_ Gaussian source ($x_0 = y_0 = 0$) and compare radial profiles at eight equally spaced angles. For the centred source, all profiles collapse onto a single curve, confirming that the spectral method preserves radial symmetry. For the off-centre source, the profiles diverge, as expected.

#figure(
  image("../figures/ch12/python/radial_symmetry_test.pdf", width: 95%),
  caption: [Radial profiles $u(r, theta_k)$ at eight angles. _Left_: with a centred Gaussian source, all profiles coincide, confirming that the spectral method preserves radial symmetry exactly. _Right_: with the off-centre source @eq-gaussian-source, the profiles differ, reflecting the broken symmetry.],
...
Adding a third variable $z in [-H, H]$ to the polar $(r, theta)$ system gives _cylindrical coordinates_ $(r, theta, z)$. If $z$ is bounded and non-periodic, it gets a Chebyshev grid; if periodic, a Fourier grid. The Laplacian becomes a three-way Kronecker product.

For _spherical coordinates_ $(r, theta, phi)$, the angular part requires _spherical harmonics_ rather than simple Fourier modes. An alternative is to treat the latitude $theta in [0, pi]$ with a Chebyshev (or Gauss--Legendre) grid and the longitude $phi in [0, 2 pi)$ with Fourier, using parity arguments at the poles analogous to our treatment of $r = 0$.

The Galerkin approach using _Zernike polynomials_ or _Jacobi polynomials_ (specifically, the so-called "one-sided Jacobi" bases) provides another route: basis functions can be chosen to satisfy the pole conditions automatically, eliminating the need for the doubling trick entirely @Shen1995 @ShenTangWang2011.

== A Non-Exhaustive Literature Overview

The use of polar coordinates in spectral methods has a rich history. The formulation we have presented, using a doubled radial variable $r in [-1, 1]$ with a symmetry condition to handle the coordinate singularity, was developed by Fornberg @Fornberg1995 @Fornberg1996 and further elaborated in his monograph @Fornberg1998. Trefethen @Trefethen2000 provided the compact and elegant implementation (Programs 28 and 29 in his book) that we have adapted and extended in this chapter.

Boyd @Boyd2000 gives a comprehensive survey of spectral methods on non-standard domains, cataloguing more than twenty approaches to the pole problem that have been proposed over the years. These range from regularity conditions and coordinate transformations to Galerkin bases that incorporate the correct behaviour at the origin. Matsushima and Marcus @Matsushima1995 developed an efficient spectral method in polar coordinates for studying vortex dynamics in rotating flows.

An important alternative to the collocation approach of this chapter is the _Galerkin_ method using basis functions that satisfy the pole conditions automatically. Shen @Shen1995 introduced families of "one-sided Jacobi" polynomial bases for the radial direction, and Shen, Tang, and Wang @ShenTangWang2011 extended these to a comprehensive framework that yields sparse operator matrices. These Galerkin methods are preferred in large-scale three-dimensional computations where the $O(N^3)$ cost of dense eigensolves becomes prohibitive.

The analytical eigenmodes of the disk involve Bessel functions, whose theory was developed in the monumental treatise of Watson @Watson1922 and tabulated by Abramowitz and Stegun @Abramowitz1972. The physical problem of vibrating circular membranes was studied systematically by Morse and Ingard @MorseIngard1968.

Applications of spectral methods on disks and in polar coordinates span a wide range of fields: quantum corrals and electron standing waves on metal surfaces @Crommie1993, vortex dynamics in geophysical flows, acoustic scattering from circular obstacles, and optical mode computation in fibre waveguides. Recent GPU-accelerated implementations have pushed polar spectral methods to large-scale three-dimensional simulations in cylindrical geometries.
...
+ *With this machinery*, eigenmodes (matching Bessel zeros to 10 digits), Poisson problems, and heat equations on the disk are solved in compact code --- each étude requiring fewer than 50 lines per language.
