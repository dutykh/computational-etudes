...
#chapter-abstract(keywords: [Periodic trapezoidal rule · Trigonometric exactness · Geometric convergence · Analyticity strip · Fourier coefficients · Fast Fourier transform])[
If exactness in the wrong space can mislead, can exactness in the right space make a humble formula look spectacular? This chapter answers yes, with the equispaced trapezoidal rule. Judged by piecewise-linear interpolation it is a clumsy first-order method, beaten even by Simpson's rule; but on a periodic interval the correct yardstick is trigonometric exactness, and by that measure the trapezoidal rule is a Fourier spectral method in disguise, exact for every trigonometric polynomial up to degree $N - 1$. The chapter develops a five-class taxonomy of its convergence --- band-limited (exact), algebraic (finite smoothness), geometric (analyticity in a strip), supergeometric (entire), and subgeometric ($C^infinity$ but not analytic) --- deriving the geometric error bound for strip-analytic integrands by both Fourier-aliasing and residue-calculus arguments. The story is traced to Poisson's 1820s computation of an ellipse's perimeter to about ten digits with only sixteen nodes, transplanted to the real line, where a Gaussian is integrated to machine precision with roughly a dozen points, and connected to the FFT, which computes Fourier coefficients as nothing more than this same trapezoidal rule. A doubled-rate observation completes the picture.
]
