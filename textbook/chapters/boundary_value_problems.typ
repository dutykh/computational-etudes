...
#figure(
  ...
  caption: [Étude 8.7: Helmholtz equation @eq-helmholtz at $k = 7$, near resonance with the $(2, 4)$ eigenmode at $k_(2,4) approx 7.02$ (four panels in a 2$times$2 grid). _(a)_: 3D surface of the solution $u(x, y)$ showing the wave-like structure. _(b)_: contour plot with the localised Gaussian forcing centre marked. _(c)_: amplitude $|u|$ at the forcing centre as $k$ sweeps over $[3, 12]$ on a semi-log axis --- the curve spikes sharply at $k = k_(2, 4)$ (coral dashed line) and at the other Dirichlet eigenvalues (faint grey lines), with smaller bumps where the forcing weakly couples to those modes. _(d)_: the $(2, 4)$ Dirichlet eigenmode itself, $sin(2 pi (x + 1) \/ 2) sin(4 pi (y + 1) \/ 2)$. Comparing (b) and (d) directly shows that the near-resonant solution *locks onto* the spatial pattern of the $(2, 4)$ mode --- this is the mechanism behind the spike in (c).],
) <fig-helmholtz>
...
#etude-conclusion[
  Panels (a) and (b) of @fig-helmholtz show the solution at $k = 7$ --- a single fixed-frequency snapshot. Panels (c) and (d) make the *mechanism* visible. The amplitude sweep in (c) spikes sharply each time $k$ crosses an eigenvalue of $-Delta$ on the unit square: when $k^2 to k_(m, n)^2 = (pi \/ 2)^2 (m^2 + n^2)$, the operator $nabla^2 + k^2 I$ becomes *nearly singular* and even a modest Gaussian source produces a response with amplitude far exceeding the source itself. Panel (d) shows *why* the showcase solution looks the way it does: the $(2, 4)$ Dirichlet eigenmode is exactly the spatial pattern that the near-resonant inverse $(nabla^2 + k^2 I)^(-1)$ amplifies most. Comparing (b) and (d) directly: the contours of the solution and the contours of the eigenmode are visibly the same shape --- the *forcing only sets the amplitude*, the eigenmode sets the geometry. The near-resonance#idx("near-resonance") regime is a *stringent test* of numerical accuracy: low-order methods need very fine meshes to resolve the large-amplitude oscillatory structure, and the ill-conditioning further degrades accuracy. The spectral method handles this gracefully ($N = 24$ suffices) because the Chebyshev basis represents the smooth eigenmode structure with few points and the direct solver sidesteps the iterative-solver difficulties for sign-indefinite systems. *Robustness near resonance* is a quiet but real advantage of spectral methods.
]
