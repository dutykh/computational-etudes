...
#chapter-abstract(keywords: [CFL condition · Stability region · Method of lines · Stiffness · Implicit methods · Integrating factor])[
A spectral discretisation that is flawless in space can explode the instant it is coupled to a time-stepping scheme: increasing the step by ten percent may turn a beautiful computation into numerical garbage. The culprit is the Courant--Friedrichs--Lewy condition, which ties the admissible time step to the spatial resolution, and for spectral methods it bites hard --- the dense spectra of Fourier and Chebyshev differentiation matrices force time-step restrictions scaling as $cal(O)(N^(-1))$, $cal(O)(N^(-2))$, or even $cal(O)(N^(-4))$. This chapter builds the geometric framework that explains these limits: write the discretisation in method-of-lines form, find the eigenvalues of the spatial operator, and ask whether the scaled spectrum lies inside the stability region of the chosen integrator. Stability regions for forward Euler, RK4, leapfrog, Adams--Bashforth, Crank--Nicolson, and BDF methods are plotted and overlaid with spectral eigenvalues. The chapter explains why Chebyshev grids are stiffer than Fourier grids, identifies the boundary-localised outlier eigenvalues responsible, and warns that for non-normal operators eigenvalues alone can mislead, motivating pseudospectra. It closes by comparing six cures for the CFL barrier --- implicit, integrating-factor, IMEX, Dufort--Frankel, Saul'yev, and hyperbolisation methods.
]
