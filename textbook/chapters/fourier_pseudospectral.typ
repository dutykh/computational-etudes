...
The exercises below progress from pencil-and-paper properties of Fourier pseudospectral discretisations, through numerical experiments that reproduce and extend the études of this chapter, to open-ended projects that reach into the current research literature. The computational problems may be carried out in any of the book's three languages; the named scripts under `codes/` give a starting point.

=== Conceptual Exercises

#exercise(title: [Symbol of the Periodic Derivative])[
  On the equispaced periodic grid of @sec-fourier-ps-deriv the Fourier pseudospectral first derivative multiplies $hat(u)_k$ by the symbol $i k$. (a) Show that this is equivalent to applying a circulant matrix $D$ whose eigenvectors are the discrete Fourier modes $bold(phi)_k$ with $(bold(phi)_k)_j = e^(i k x_j)$ and whose eigenvalues are the symbols $i k$, $k in K_N$. (b) Conclude that the spectrum of $D$ is purely imaginary, that $D$ is skew-symmetric, $D^top = -D$, and that $D bold(1) = bold(0)$. (c) Show that the second-derivative operator carries the real, non-positive symbol $-k^2$, and explain why the first- and second-derivative operators commute.
] <ex-fps-symbol>

#exercise(title: [The Nyquist Mode and Odd Derivatives])[
  For even $N$ the wavenumber set $K_N$ contains the Nyquist mode $k = N\/2$, sampled as $cos(N x_j \/ 2) = (-1)^j$ on the grid. (a) Explain why a real grid function fixes its Nyquist content only through the cosine $cos(N x \/ 2)$, while the companion sine $sin(N x \/ 2)$ vanishes at every node and is invisible to the grid. (b) Show that assigning the derivative symbol $i (N\/2)$ to this mode would send the real vector $((-1)^j)$ to a purely imaginary result, breaking the skew-symmetry of @ex-fps-symbol; deduce that the first-derivative symbol of the Nyquist mode must be set to zero, exactly the zero placed in the Nyquist slot of the wavenumber vector `k` in the étude code of @etude-fourier-diff. (c) Show that for the second derivative the Nyquist symbol $-(N\/2)^2$ is instead retained, because it maps real vectors to real vectors.
] <ex-fps-nyquist>

#exercise(title: [CFL Scaling on Periodic Grids])[
  The semidiscrete system @eq-mol is advanced with an explicit Runge--Kutta method whose stability region reaches a distance $R$ along both the imaginary and the negative real axes. (a) For pure advection $u_t + c u_x = 0$ the eigenvalues are $lambda_k = -i c k$ with $|lambda_max| = |c| N \/ 2$; deduce the restriction $Delta t lt.eq.slant 2 R \/ (|c| N)$, that is $Delta t tilde.op 1 \/ N$. (b) For pure diffusion $u_t = kappa u_(x x)$ the eigenvalues are $lambda_k = -kappa k^2$ with $|lambda_max| = kappa (N \/ 2)^2$; deduce $Delta t tilde.op 1 \/ (kappa N^2)$. (c) Contrast these with the Chebyshev grid, where boundary clustering forces $Delta t tilde.op N^(-2)$ for advection and $Delta t tilde.op N^(-4)$ for diffusion, and explain in one sentence why the periodic grid is so much friendlier to explicit stepping.
] <ex-fps-cfl>

#exercise(title: [Aliasing and the Two-Thirds Rule])[
  A quadratic nonlinearity multiplies two grid functions pointwise before transforming back to Fourier space. (a) Show that the pointwise product of $e^(i k_1 x_j)$ and $e^(i k_2 x_j)$ on the $N$-point grid is indistinguishable from the single mode $e^(i (k_1 + k_2) x_j)$, and that when $|k_1 + k_2| > N \/ 2$ this sum wavenumber is aliased to $k_1 + k_2 minus.plus N$ inside the resolved band. (b) Deduce that retaining only modes with $|k| lt.eq.slant N \/ 3$ before forming the product keeps every genuine sum wavenumber within $|k| lt.eq.slant 2 N \/ 3$, too small to alias into the kept band, which is the mask @eq-dealias-mask. (c) Show that the equivalent zero-padding rule needs a fine grid of at least $M = 3 N \/ 2$ points, and explain why a cubic nonlinearity requires the stricter ratio $M = 2 N$.
] <ex-fps-aliasing>

#hint-for(<ex-fps-aliasing>)[On the $N$-point grid two wavenumbers are indistinguishable when they differ by a multiple of $N$, so the sum $k_1 + k_2$ is read modulo $N$ back into $(-N \/ 2, N \/ 2]$. Bounding both factors by $N \/ 3$ keeps every sum within $|k| lt.eq.slant 2 N \/ 3$, which stays more than $N \/ 3$ away from the nearest alias and so cannot wrap into the retained band.]

#exercise(title: [Why the Integrating Factor Removes Stiffness])[
  Consider the semidiscrete system @eq-linear-nonlinear, $hat(u)_k '(t) = lambda_k hat(u)_k (t) + hat(cal(N))_k (t)$, in which the linear symbol $lambda_k$ grows like $i k^3$ (KdV) or $k^2 - k^4$ (Kuramoto--Sivashinsky) and carries the stiffness. (a) Introduce the integrating-factor variable $hat(v)_k (t) = e^(-lambda_k t) hat(u)_k (t)$ and derive @eq-if-ode, $hat(v)_k '(t) = e^(-lambda_k t) hat(cal(N))_k (t)$, showing that the stiff linear term has been removed entirely. (b) Show that when $hat(cal(N))_k equiv 0$ the transformed equation is solved exactly for any step size, so the integrating factor reproduces the exact propagator @eq-exact-integration. (c) Explain why the per-step form @eq-ifrk4, built from $E = e^(lambda_k Delta t \/ 2)$ and $E_2 = e^(lambda_k Delta t)$, avoids the overflow that a cumulative factor $e^(-lambda_k t)$ would produce when $lambda_k$ has a large negative real part.
] <ex-fps-if>

#exercise(title: [Order and Unitarity of Strang Splitting])[
  The split-step method for the nonlinear Schrödinger equation @eq-nls advances the linear flow $e^(Delta t cal(A))$ and the nonlinear flow $e^(Delta t cal(B))$ in the symmetric order $e^(Delta t cal(A) \/ 2) thin e^(Delta t cal(B)) thin e^(Delta t cal(A) \/ 2)$. (a) Using the Baker--Campbell--Hausdorff expansion, show that the unsymmetric (Lie) composition $e^(Delta t cal(A)) thin e^(Delta t cal(B))$ is first-order accurate, with leading error proportional to the commutator $[cal(A), cal(B)]$, while the symmetric (Strang) composition is second-order, with leading error governed by the double commutators $[cal(A), [cal(A), cal(B)]]$ and $[cal(B), [cal(A), cal(B)]]$. (b) Show that the linear half-step $hat(u)_k arrow.l e^(-i k^2 Delta t \/ 2) hat(u)_k$ and the nonlinear step $u arrow.l e^(2 i |u|^2 Delta t) u$ each preserve the discrete $L^2$ norm exactly. (c) Conclude that the composite scheme conserves $|| u ||_2$ to roundoff independently of $Delta t$, as observed in @etude-nls.
] <ex-fps-strang>

#exercise(title: [The KdV Soliton and Its Invariants])[
  Consider the KdV equation @eq-kdv, $u_t + 6 u u_x + u_(x x x) = 0$. (a) Substitute the travelling-wave ansatz $u(x, t) = f(x - c t)$, integrate the resulting ordinary differential equation twice under the decay conditions $f, f', f'' arrow.r 0$ at infinity, and recover the soliton @eq-soliton, $u = 2 a^2 op("sech")^2 (a (x - x_0 - 4 a^2 t))$, with speed $c = 4 a^2$. (b) Verify that the amplitude $2 a^2$ and the speed $4 a^2$ are linked, so that taller solitons travel faster. (c) Show that $I_1 = integral u dif x$ and $I_2 = integral u^2 dif x$ are conserved by smooth periodic solutions, and derive the linear dispersion relation $omega(k) = -k^3$ of the linearised equation $u_t + u_(x x x) = 0$, distinguishing phase and group velocity.
] <ex-fps-kdv-invariants>

#hint-for(<ex-fps-kdv-invariants>)[Substitute $u = f(xi)$ with $xi = x - c t$ and integrate once to obtain $f'' - c f + 3 f^2 = "const"$; the decay conditions set the constant to zero. Multiply by $f'$ and integrate again to reach $(f')^2 = c f^2 - 2 f^3$, whose decaying solution is the sech-squared profile @eq-soliton with $c = 4 a^2$.]

#exercise(title: [Linear Instability of Kuramoto--Sivashinsky])[
  The Kuramoto--Sivashinsky equation @eq-ks has linear symbol $lambda_k = k^2 - k^4$. (a) Linearise @eq-ks about the zero state and show that the mode $e^(i k x)$ grows like $e^(lambda_k t)$, so that wavenumbers with $0 < |k| < 1$ are linearly unstable while those with $|k| > 1$ are damped. (b) Maximise $lambda_k$ over $k$ to find the most unstable wavenumber $k_* = 1 \/ sqrt(2)$ and the corresponding cell wavelength $2 pi \/ k_* = 2 sqrt(2) thin pi$. (c) Explain why $u_(x x)$ acts as a negative diffusion that injects energy at long wavelengths while $u_(x x x x)$ supplies the short-wavelength damping that arrests the growth, and estimate how many linearly unstable modes are present on the domain $L = 32 pi$ of @etude-ks.
] <ex-fps-ks-instability>

=== Computational Exercises

#exercise(title: [Fourier Differentiation: Accuracy and Cost])[
  Reproduce and extend @etude-fourier-diff with the script `fourier_diff_comparison`. (a) Differentiate $u(x) = e^(sin x)$ on $[0, 2 pi)$ using both the dense circulant matrix $D$ and the FFT route $cal(F)^(-1)(i k hat(u))$, and confirm that the maximum error of each falls to machine precision by $N approx 30$. (b) Repeat for the rougher functions $f(x) = |sin x|^3$ and a sawtooth wave, and explain why the convergence becomes algebraic rather than exponential. (c) Time both routes for $N = 2^4, 2^5, dots, 2^(12)$ and verify the $O(N^2)$ versus $O(N log N)$ scalings of @fig-fourier-diff-timing. (d) Add the spectral second derivative through the symbol $-k^2$ and check it against $u'' (x)$ for the smooth test function.
] <ex-fps-diff-cost>

#exercise(title: [Dealiasing Effect on Long-Time Burgers Simulation])[
  Solve the viscous Burgers equation $u_t + u u_x = nu u_(x x)$ on $[0, 2pi]$ with $nu = 0.01$ and $u(x, 0) = sin(x)$ using the Fourier pseudospectral method with RK4 time stepping. (a) Run with $N = 64$ and $Delta t = 10^(-3)$ to $t = 5$, both with and without the $2\/3$ dealiasing rule. (b) Plot the Fourier spectrum $|hat(u)_k|$ at $t = 1, 3, 5$ for both cases. (c) Identify the aliased energy that contaminates the low modes in the undealiased case. (d) Compute $||u_(Delta t\/2) - u_(Delta t)||_infinity$ at $t = 5$ for both dealiased and undealiased cases to assess the practical impact on accuracy.
] <ex-fps-burgers-dealias>

#exercise(title: [KdV Soliton Propagation])[
  Implement a Fourier pseudospectral solver for the KdV equation $u_t + 6 u u_x + u_(x x x) = 0$ on a periodic domain $[0, L]$ with $L = 40$. Use the initial condition $u(x, 0) = 2 "sech"^2(x - 10)$ (a single soliton of speed $c = 4$). (a) Solve using the integrating factor method with RK4. (b) Verify that the soliton propagates at speed $c = 4$ by plotting $u(x, t)$ at $t = 0, 2, 4, 6$. (c) Compute the conserved quantities $I_1 = integral u dif x$ and $I_2 = integral u^2 dif x$ at each time step; verify that the mass $I_1$, carried by the untouched zero mode, holds to machine precision, while the momentum $I_2$ drifts only at the order of the RK4 time-stepping error. (d) Repeat with two solitons ($c_1 = 16$ and $c_2 = 4$) and observe elastic collision.
] <ex-fps-kdv-propagation>

#exercise(title: [Kuramoto--Sivashinsky: Stiffness and Spectrum])[
  Using the script `kuramoto_sivashinsky`, study the stiff dynamics of @eq-ks with the per-step IF-RK4 scheme @eq-ifrk4. (a) On the domain $L = 32 pi$ with $N = 256$, reproduce the space-time fingerprint of @etude-ks and the time-averaged energy spectrum, confirming that energy concentrates in the linearly unstable band $|k| < 1$. (b) Replace IF-RK4 by plain explicit RK4 and find, by experiment, the largest stable $Delta t$; verify that it shrinks like $N^(-4)$ and is orders of magnitude below the IF-RK4 step, which is limited only by the nonlinear CFL condition. (c) Vary the domain length $L$ and locate the approximate onset of sustained chaos, relating it to the count of linearly unstable modes from @ex-fps-ks-instability. (d) Confirm that dealiasing is active by comparing the high-wavenumber tail of the spectrum with and without the two-thirds rule.
] <ex-fps-ks-stiffness>

#exercise(title: [Energy Conservation in a Pseudospectral Schrödinger Solver])[
  Solve the cubic nonlinear Schrödinger equation $i u_t + u_(x x) + 2|u|^2 u = 0$ on $[0, 2pi]$ with initial condition $u(x, 0) = "sech"(x - pi) e^(2 i x)$ using the split-step Fourier method. (a) Run with $N = 256$ and $Delta t = 10^(-3)$ to $t = 10$. (b) Monitor three conserved quantities: mass $M = integral |u|^2 dif x$, momentum $P = "Im" integral overline(u) u_x dif x$, and energy $H = integral (|u_x|^2 - |u|^4) dif x$. (c) Plot the relative error in each invariant versus time. (d) Compare Strang splitting ($Delta t$ and $Delta t\/2$ half-steps) with Lie splitting (full step of each operator) and show that Strang splitting preserves the invariants more accurately.
] <ex-fps-nls-invariants>

#exercise(title: [Allen--Cahn Equation with IMEX Splitting])[
  Solve the Allen--Cahn equation $u_t = epsilon^2 u_(x x) + u(1 - u^2)$ on $[0, 2pi]$ with $epsilon = 0.1$ and random initial data $u(x, 0) = 0.1 xi(x)$ where $xi$ consists of $N\/4$ random Fourier modes. (a) Implement the IMEX scheme: treat the diffusion term implicitly in Fourier space and the nonlinear reaction term explicitly. (b) Run with $N = 256$ and $Delta t = 0.01$ until $t = 50$. (c) Plot the solution at several times and describe the coarsening dynamics. (d) Measure the discrete energy $E = integral (epsilon^2\/2 |u_x|^2 + (1 - u^2)^2\/4) dif x$ and verify that it decreases monotonically.
] <ex-fps-allen-cahn-imex>

=== Project-Style Exercises

#exercise(title: [Two-Dimensional Turbulence and the Dual Cascade])[
  Extend the vorticity--streamfunction solver of @etude-ns2d (script `navier_stokes_2d`) into a study of two-dimensional turbulence. (a) Starting from the decaying-turbulence setup with $nu = 10^(-3)$ on $[0, 2 pi)^2$, compute the kinetic-energy and enstrophy spectra and track the inverse energy cascade toward large scales together with the selective decay of enstrophy predicted by Kraichnan @Kraichnan1967. (b) Add a small-scale random forcing to reach a statistically steady state and look for the dual cascade: an inverse energy cascade with a $k^(-5 \/ 3)$ range and a direct enstrophy cascade with a $k^(-3)$ range. (c) Push the resolution as far as the hardware allows and discuss the roles of dealiasing and of the time-step restriction at high $N$, comparing with the GPU-accelerated enstrophy-cascade study of Boffetta _et al._ @Boffetta2025 and the spectral-methods treatment of Canuto _et al._ @Canuto2006.
] <ex-fps-ns2d-turbulence>

#hint-for(<ex-fps-ns2d-turbulence>)[In two dimensions both energy and enstrophy are inviscid invariants; their simultaneous conservation drives energy toward large scales and enstrophy toward small scales, the origin of the $k^(-5 \/ 3)$ and $k^(-3)$ ranges. Diagnose the cascades from the spectral fluxes, not from the spectra alone.]

#exercise(title: [Exponential Time Differencing versus the Integrating Factor])[
  The per-step IF-RK4 scheme @eq-ifrk4 used throughout @sec-integrating-factor is the simplest exponential integrator, but not the most accurate. (a) Implement the fourth-order exponential time-differencing scheme ETDRK4#idx("ETDRK4") of Cox and Matthews @CoxMatthews2002, evaluating the entire functions $phi_j (lambda_k Delta t)$ that appear in its weights. (b) Reproduce the contour-integral remedy of Kassam and Trefethen @KassamTrefethen2005 for the cancellation errors that afflict these weights when $lambda_k Delta t approx 0$, and confirm that it restores full accuracy. (c) Compare ETDRK4 and IF-RK4 on the KdV equation @eq-kdv and the Kuramoto--Sivashinsky equation @eq-ks at fixed cost, measuring the error against a highly resolved reference, and discuss when the extra complexity of ETDRK4 pays off. (d) As an extension, study the energy-decreasing higher-order ETD Runge--Kutta methods for gradient flows of Fu, Shen, and Yang @FuShenYang2024.
] <ex-fps-etdrk4>

#hint-for(<ex-fps-etdrk4>)[The ETDRK4 weights are combinations of the entire functions $phi_1, phi_2, phi_3$ of $z = lambda_k Delta t$, and evaluating them directly suffers catastrophic cancellation near $z = 0$. Kassam and Trefethen replace each weight by the average of its values at points on a small circle in the complex $z$-plane, a contour integral that is numerically stable.]

#exercise(title: [Structure-Preserving Integration of the NLS Equation])[
  The split-step method of @etude-nls preserves the $L^2$ norm but not the Hamiltonian of the nonlinear Schrödinger equation @eq-nls exactly. (a) Implement the split-step Fourier method and a recent energy-preserving exponential wave integrator, following the uniform error analysis of Li and Qin @LiQin2025, and compare the long-time drift of mass, momentum, and energy. (b) Study the semiclassical regime in which the dispersion carries a small parameter, where Bao, Jin, and Markowich @BaoJinMarkowich2002 showed that time-splitting spectral schemes capture the correct observables under mild step restrictions, and reproduce their meshing-strategy conclusions numerically. (c) Compare Strang splitting with a higher-order (for example Yoshida) composition and with the energy-preserving integrator on a soliton and on a modulationally unstable plane wave, building on the foundational analyses of Hardin and Tappert @HardinTappert1973 and Taha and Ablowitz @Taha1984.
] <ex-fps-nls-structure>

#hint-for(<ex-fps-nls-structure>)[Each split-step substep is the exact flow of a sub-Hamiltonian, so the method is symplectic and conserves $|| u ||_2$ while letting the energy oscillate without secular drift; an energy-preserving integrator instead pins the discrete Hamiltonian and lets a different invariant move. Decide which invariant matters most for the observable you report.]

#exercise(title: [Energy-Stable Schemes for the Allen--Cahn Gradient Flow])[
  The Allen--Cahn equation @eq-allen-cahn is the $L^2$ gradient flow of the Ginzburg--Landau free energy, and a good integrator should respect its energy dissipation. (a) Reproduce the IMEX étude @etude-allen-cahn (script `allen_cahn`) and verify numerically that the discrete free energy decreases monotonically along the coarsening dynamics. (b) Implement the convex-splitting scheme of Eyre @Eyre1998 and the unconditionally energy-stable spectral scheme of Shen and Yang @ShenYang2010, and compare the largest stable time step and the measured energy decay against the plain IMEX scheme. (c) Investigate the coarsening law by tracking the number of interfaces or the characteristic domain size over time, and discuss how the width parameter $epsilon$ and the resolution $N$ must be related to resolve the $O(epsilon)$ transition layers.
] <ex-fps-allen-cahn-energy>

#hint-for(<ex-fps-allen-cahn-energy>)[Convex splitting writes the free energy as a difference of two convex parts and treats the contractive part implicitly and the expansive part explicitly, which guarantees an unconditional energy decrease. In Fourier space the implicit solve remains a diagonal division, so the cost per step is essentially that of the plain IMEX scheme.]
