...
We solve the ODE by collocation on the interior TL_n-like grid
    y_i = L cot^2(t_i / 2),   t_i = (2i - 1) pi / (2 N + 2), i = 1,...,N+1
...
def sb_basis(n, y, L=3.0):
    """SB_n basis: sin[(n+1) arccot(y/L)], evaluated at arbitrary y.

    Using the map t = arccot(y/L), y = L cot(t), t in (0, pi).
    SB_n(y) = sin((n+1) t).
    """
    t = np.arctan(L / y)              # arccot(y/L) = arctan(L/y)
    # careful: arctan maps y in R to t in (0, pi); when y > 0, t in (0, pi/2);
    # for y < 0, numpy.arctan(L/y) in (-pi/2, 0), but we need t in (pi/2, pi).
    # Compose: t = pi/2 - arctan(y/L).
    t = np.pi / 2.0 - np.arctan(y / L)
    return np.sin((n + 1) * t)


def sb_deriv(n, y, L=3.0):
    """y-derivative of SB_n(y; L) via chain rule.  t = pi/2 - arctan(y/L),
    so dt/dy = -1 / (L (1 + (y/L)^2)) = -L / (L^2 + y^2).
    """
    t = np.pi / 2.0 - np.arctan(y / L)
    return (n + 1) * np.cos((n + 1) * t) * (-L / (L ** 2 + y ** 2))


def sb_second_deriv(n, y, L=3.0):
    t = np.pi / 2.0 - np.arctan(y / L)
    denom = L ** 2 + y ** 2
    dt_dy = -L / denom
    d2t_dy2 = 2.0 * L * y / denom ** 2
    c = np.cos((n + 1) * t)
    s = np.sin((n + 1) * t)
...
def _sbn_reference_solution(y):
    if "ref" not in _ref_cache:
        c, idx = assemble_and_solve(21, L=3.0)
        _ref_cache["ref"] = (c, idx)
    c, idx = _ref_cache["ref"]
    return evaluate_solution(c, idx, y, L=3.0)
