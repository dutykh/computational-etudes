...
function laguerre_poly(n, y)
    yv = collect(y)
    L = zeros(n + 1, length(yv))
    L[1, :] .= 1.0
    if n >= 1
        L[2, :] .= 1.0 .- yv
    end
    for k in 1:(n - 1)
        L[k + 2, :] .= ((2k + 1 .- yv) .* L[k + 1, :] .- k .* L[k, :]) ./ (k + 1)
    end
    return L
end
...
function laguerre_error(f, N)
    ...
    c = vec(sum(L .* (w .* exp.(x ./ 2) .* fv)', dims=2))
...
function tln_error(f, N, L)
    _, x = cheb_matrix(N)
    fv = zeros(length(x))
    ok = abs.(x) .< 1.0 - 1e-12
    y_ok = L .* (1 .+ x[ok]) ./ (1 .- x[ok])
    fv[ok] .= f.(y_ok)
    a = dct1_coeffs(fv)
    y_fine = collect(range(0.001, 60.0, length=4001))
    x_fine = (y_fine .- L) ./ (y_fine .+ L)
...
function run()
    ...
    scatterlines!(ax1, Ns, max.(err_tln_e, 1e-18); color=TEAL, label="TL_n L=2")
    ...
    scatterlines!(ax2, Ns, max.(err_tln_a, 1e-18); color=TEAL, label="TL_n L=5")
