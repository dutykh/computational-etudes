...
function run()
    ...
    for (i, N) in enumerate(Ns), (j, L) in enumerate(Ls)
        E[i, j] = eval_error(N, L, y_eval, truth)
    end
    ...
    Ng = 32
    for (L, col, off) in [(0.1, CORAL, -0.08), (0.3, TEAL, -0.18), (1.0, ORANGE, -0.28)]
        x = [-pi + 2pi * k / Ng for k in 0:Ng-1]
        yc = 2.0 .* atan.(L .* tan.(x ./ 2))
        scatter!(ax1, yc, fill(off, length(yc)); color=col, markersize=6)
    end

    ax2 = Axis(fig[1, 2], xlabel="log10 L", ylabel="N", title="(b) Error landscape")
    heatmap!(ax2, log10.(Ls), Ns, log10.(E .+ 1e-16)')
    Colorbar(fig[1, 2][1, 2], limits=extrema(log10.(E .+ 1e-16)))

    ax3 = Axis(fig[1, 3], xlabel="L", ylabel="max error", xscale=log10,
