...
"""
    rational_chebyshev_grid(N, L = 4.0)
...
"""
function rational_chebyshev_grid(N::Int, L::Real = 4.0)
    D_t, t = cheb_matrix(N)
    t_int = t[2:N]
    x_int = L .* t_int ./ sqrt.(1.0 .- t_int .^ 2)
    return x_int, t, D_t
end

"""
    rational_chebyshev_derivative_matrices(N, L = 4.0)
...
"""
function rational_chebyshev_derivative_matrices(N::Int, L::Real = 4.0)
...
if abspath(PROGRAM_FILE) == @__FILE__
    ...
    for N in (16, 32, 64)
        ...
        @printf("N = %3d  L = 4   max error in u_xx (Gaussian) = %.3e\n", N, err)
    end
end
