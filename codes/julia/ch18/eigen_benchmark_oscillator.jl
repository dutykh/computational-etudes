...
function solve_oscillator(N::Int, ell::Real)
...
function make_figure(; Ns = (16, 32), ell_scan = (2.0, 4.0, 8.0), ell_best::Real = 4.0,
                     dump_path = nothing)
    fig = Figure(size = (1100, 450))

    # Panel A: N-scan at fixed ell_best
    ax1 = Axis(fig[1, 1];
    ...
    # Panel B: ell-scan at fixed N
    ax2 = Axis(fig[1, 2];
        xlabel = "mode number j",
        ylabel = "|λ_num − λ_exact|",
        title  = "ℓ-scan at N = 32: map-parameter sensitivity",
        yscale = log10, limits = (nothing, (1e-17, 1e4)))
    markers2 = [:utriangle, :circle, :dtriangle]
    cols2 = [SKY, NAVY, PURPLE]
    counts_by_ell = Dict{Float64,Int}()
    N_scan = 32
    for (ell, m, c) in zip(ell_scan, markers2, cols2)
        ...
        scatterlines!(ax2, j .+ 1, err_plot;
                      ...
                      linewidth = 0.6, marker = m, markersize = 8,
                      label = "ℓ = $ell")
        counts_by_ell[ell] = count_good(err, 1e-2)
    end
    ...
    for N in Ns
        spectra["N$(N)_ell$(ell_best)"] = solve_oscillator(N, ell_best)
    end
    for ell in ell_scan
        spectra["N$(N_scan)_ell$(ell)"] = solve_oscillator(N_scan, ell)
    end

    r = Dict{String,Any}(
        "Ns"             => collect(Ns),
        "ell_best"       => ell_best,
        "ell_scan"       => collect(ell_scan),
        "counts_by_N"    => Dict(string(k) => v for (k, v) in counts_by_N),
        "counts_by_ell"  => Dict(string(k) => v for (k, v) in counts_by_ell),
        "spectra"        => spectra,
    )
    dump_path !== nothing && write_json(dump_path, r)
    return r, counts_by_N, counts_by_ell
end
...
if abspath(PROGRAM_FILE) == @__FILE__
    dp = parse_args()
    _, cN, c_ell = make_figure(; dump_path = dp)
    println("[Etude 18.4]  harmonic oscillator on (-inf, +inf)")
    println("  ell = 4.0 scan over N:")
    for N in sort(collect(keys(cN)))
        @printf("    N = %2d:  %d good eigenvalues (|err| < 1e-2)\n", N, cN[N])
    end
    println("  N = 32 scan over ell:")
    for ell in sort(collect(keys(c_ell)))
        @printf("    ell = %.1f:  %d good eigenvalues\n", ell, c_ell[ell])
    end
